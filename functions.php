...
function start_auto_payment_check($user_id, $amount, $message_id) {
    ...
    if($stmt->rowCount() > 0) {
        return [
            'status' => false,
            'message' => "این پرداخت قبلاً تأیید شده است و به کیف پول شما اضافه شده است."
        ];
    }
    
    // ذخیره اطلاعات بررسی خودکار در دیتابیس
    $check_id = bin2hex(random_bytes(5));
    $start_time = date('Y/m/d H:i:s');
    $end_time = date('Y/m/d H:i:s', strtotime('+15 minutes'));
    
    $stmt = $pdo->prepare("INSERT INTO auto_payment_checks (check_id, user_id, amount, start_time, end_time, status) VALUES (?, ?, ?, ?, ?, 'pending')");
    $stmt->bindParam(1, $check_id);
    ...
    // ایجاد کیبورد جدید برای نمایش وضعیت بررسی
    $keyboard = json_encode([
        'inline_keyboard' => [
            [
                ['text' => '🔄 در حال بررسی خودکار...', 'callback_data' => 'check_status_' . $check_id],
                ['text' => '🔍 بررسی دستی', 'callback_data' => 'manual_check_' . $amount]
            ]
        ]
    ]);
    
    // ویرایش پیام فعلی
    Editmessagetext($user_id, $message_id, sprintf($textbotlang['users']['moeny']['auto_check_started'], number_format($amount)), $keyboard);
...
function check_auto_payment($check_id) {
    global $pdo, $textbotlang;
    
    // دریافت اطلاعات بررسی
    $stmt = $pdo->prepare("SELECT * FROM auto_payment_checks WHERE check_id = ?");
    $stmt->bindParam(1, $check_id);
    $stmt->execute();
    ...
    // بررسی پرداخت
    $payment_result = check_payment_status($check['user_id'], $check['amount']);
    ...
    if ($payment_result['status']) {
        ...
        $user = select("user", "*", "id", $check['user_id'], "select");
        $Balance_confrim = intval($user['Balance']) + intval($check['amount']);
        update("user", "Balance", $Balance_confrim, "id", $check['user_id']);
        ...
        $stmt = $pdo->prepare("INSERT INTO Payment_report (id_user, id_order, time, price, payment_Status, Payment_Method, transaction_details) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bindParam(1, $check['user_id']);
        $stmt->bindParam(2, $randomString);
        $stmt->bindParam(3, $dateacc);
        $stmt->bindParam(4, $check['amount']);
        $stmt->bindParam(5, $payment_Status);
        $stmt->bindParam(6, $Payment_Method);
        $transaction_details = json_encode($transaction);
        $stmt->bindParam(7, $transaction_details);
        $stmt->execute();
        
        // به‌روزرسانی وضعیت بررسی
        $stmt = $pdo->prepare("UPDATE auto_payment_checks SET status = 'success', completed_at = NOW() WHERE check_id = ?");
        $stmt->bindParam(1, $check_id);
        $stmt->execute();
        
        // ویرایش پیام برای کاربر
        ...
        $success_message = sprintf($textbotlang['users']['moeny']['Charged.'], number_format($check['amount']), $randomString);
        Editmessagetext($check['user_id'], $check['message_id'], $success_message, $success_keyboard);
        
        return true;
    }
    
    // بررسی آیا زمان بررسی به پایان رسیده است
    if (strtotime($check['end_time']) < time()) {
        // به‌روزرسانی وضعیت بررسی به ناموفق
        $stmt = $pdo->prepare("UPDATE auto_payment_checks SET status = 'failed', completed_at = NOW() WHERE check_id = ?");
        $stmt->bindParam(1, $check_id);
        $stmt->execute();
        
        // ویرایش پیام برای کاربر
        $failed_keyboard = json_encode([
            'inline_keyboard' => [
                [
                    ['text' => '🔍 بررسی دستی', 'callback_data' => 'manual_check_' . $check['amount']],
                    ['text' => '🏠 بازگشت به منوی اصلی', 'callback_data' => 'gotohome']
                ]
            ]
        ]);
        
        Editmessagetext($check['user_id'], $check['message_id'], "⏰ زمان بررسی خودکار به پایان رسید. لطفاً بررسی دستی را امتحان کنید یا با پشتیبانی تماس بگیرید.", $failed_keyboard);
